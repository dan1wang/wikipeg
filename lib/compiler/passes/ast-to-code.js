...
function generateJavascript(ast, options) {
  ...
  /**
   * Handler for simple_and, simple_not, discarded_and and discarded_not.
   */
  function buildSimplePredicate(node, context) {
    var result = new Result();
    var negate = (node.type === 'simple_not') || (node.type === 'discarded_not');
  ...
  /**
   * Handler for discarded_and and discarded_not.
   */
  function buildDiscardedPredicate(node, context) {
    var result = buildSimplePredicate(node, context);
  ...
  var generate = visitor.build({
    ...
    discarded_and: buildDiscardedPredicate,
    discarded_not: buildDiscardedPredicate,
