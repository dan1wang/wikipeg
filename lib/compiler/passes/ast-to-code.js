  ...
  var generate = visitor.build({
    ...
    sequence: function(node, context) {
      if (node.elements.length === 1) {
        ...
        for (i = 0; i < node.elements.length; i++) {
          if (node.elements[i].type === 'picked') {
            hasPicked = true;
            break;
          }
        }
        ...
        for (i = 0; i < node.elements.length; i++) {
          var subresult;
          // If a sequence has @ operator anywhere, discard every element that
          // is not picked. Note labeled expessions will un-discard themselves.
          if (hasPicked && (node.elements[i].type !== 'picked')) {
            subresult = recurse(node.elements[i], context.noPassThru().discard());
          } else {
            subresult = recurse(node.elements[i], context.noPassThru());
          }
          subresult.free = [];
          result.append(subresult);
          if (!hasPicked || (node.elements[i].type === 'picked')) {
        ...
        if (context.getDiscard()) {
          ...
          if (resultExpressions.length > 1) {
            ...
            result.block.push(`${resultReg} = ${resultExpressions[0]};`);
          }
        }
        result.block.push(language.blockEnd(label));
    ...
    picked: function(node, context) {
      ...
      if (node.expression.type == 'labeled') {
        // do not create a new context as it would impact scope of label variables
        newContext = context;
      } else {
        newContext = context.cloneEnv().discard(false);
      }
      result.append(recurse(node.expression, newContext));
      result.picked = true;
      return result;
