...
function generateJavascript(ast, options) {
  ...
  Context.prototype = {
    ...
    /**
     * Clone the object, and set the asserted flag in the cloned object. This
    ...
    /**
     * Get the asserted flag.
     */
    getAsserted: function() {
   ...
   */
  function addRule(name, asserted) {
    const funcName = language.ruleFuncName(name, asserted, false);
  ...
  function makeFailCall(value, context) {
    ...
    if (silence === 'true') {
      throw new Error("Failure reporting optimization should have removed unnecessary peg$fail() call.");
    }
    ...
    if (silence === 'false') {
      ...
      if (context.getAsserted()) {
        // assertion rule: always reports failure
        return call + ';';
      } else {
  ...
  function buildSimplePredicate(node, context) {
    ...
    var newContext = context.silence().cloneEnv().asserted();
   ...
   */
  function getRuleArgNames(rule, asserted) {
    var args;
    if (asserted) {
      args = []; // assertion rule: always reports failure
    } else {
      args = rule.reportsFailure?[language.silence]:[];
    }
  ...
  var generate = visitor.build({
    rule: function(node, funcName, asserted, iterable) {
      ...
      // Generate the Result
      let context;
      if (node.reportsFailure) {
        context = (new Context()).asserted(asserted);
      } else {
        context = (new Context()).asserted(asserted).silence();
      }
      var result;
      if (iterable) {
      ...
      if (iterable) {
        ...
        if (options.cache) {
          ...
          cacheBits = cacheFunc({
            ...
            variantIndex: asserted ? 1 : 0,
            variantCount: 2,
            variantName: asserted ? 'asserted' : 'normal',
      ...
      let argNames = getRuleArgNames(node, asserted);
    ...
    rule_ref: function(node, context) {
      ...
      let argNameList = getRuleArgNames(rule, context.getAsserted());
      let args = [];
      ...
      let funcName = addRule(node.name, context.getAsserted());
    ...
    named: function(node, context) {
      var result = new Result();
      if (node.reportsFailure) {
        // this is a start rule (always report error)
        result.append(recurse(node.expression, context.cloneEnv()));
      } else {
        // this is not a start rule (silence error reporting)
    ...
    action: function(node, context) {
      var result = new Result();
      var reg = context.getResultReg(result);
      var newContext = context.cloneEnv().asserted();
    ...
    sequence: function(node, context) {
      if (node.elements.length === 1) {
        ...
        for (i = 0; i < node.elements.length; i++) {
          ...
          subresult.free = [];
          result.append(subresult);
          resultExpressions.push(subresult.expression);
          if (subresult.picked) pickedExpressions.push(subresult.expression);
        ...
        if (context.getAsserted()) {
          ...
          switch (pickedExpressions.length) {
            case 0:
              result.block.push(`${resultReg} = [${resultExpressions.join(',')}];`);
              break;
            case 1:
              result.block.push(`${resultReg} = ${pickedExpressions[0]};`);
              break;
            default:
              result.block.push(`${resultReg} = [${pickedExpressions.join(',')}];`);
          }
        }
        result.block.push(language.blockEnd(label));
    ...
    labeled: function(node, context) {
      var reg = allocReg([]);
      var newContext = context.cloneEnv().resultReg(reg).asserted(false);
      var subresult = recurse(node.expression, newContext);
    ...
    picked: function(node, context) {
      const result = new Result();
      const newContext = context.cloneEnv().asserted(false);
      result.append(recurse(node.expression, newContext));
      result.picked = true;
      return result;
    },
    ...
    text: function(node, context) {
      ...
      result.append(recurse(node.expression, context.cloneEnv().asserted()));
      result.onSuccess([
    ...
    zero_or_more: function(node, context) {
      // Pseudocode for the normal (non-asserted) case:
      //
      // let r1 = [];
      // while (true) {
      //   let r2 = expr();
      ...
      // }
      //
      // Pseudocode for the asserted case:
      //
      // while (true) {
      //   let r2 = expr();
      //   if (r2 === failed) {
  ...
  while (rulesToGenerate.length) {
    ...
    generated.push(generate(rule, ruleInfo.funcName, ruleInfo.asserted, false));
  }
