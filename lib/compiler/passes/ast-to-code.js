...
function generateJavascript(ast, options) {
  ...
  /**
   * Handler for simple_and, simple_not, discardable_and and discardable_not.
   */
  function buildSimplePredicate(node, context) {
    var result = new Result();
    var negate = (node.type === 'simple_not') || (node.type === 'discardable_not');
  ...
  /**
   * Handler for discardable_and and discardable_not.
   */
  function buildDiscardablePredicate(node, context) {
    var result;
    if ((node.type === 'discardable_and') || (node.type === 'discardable_not')) {
      result = buildSimplePredicate(node, context);
    } else {
      result = new Result();
      var resultReg = context.getResultReg(result);
      var newContext = context.silence().cloneEnv().discard();
      result.append(recurse(node.expression, newContext));
      result.onSuccess([`${resultReg} = ${language.assertionSuccess};`]);
    }
  ...
  var generate = visitor.build({
      ...
      if (node.elements.length === 1) {
        ...
        if (context.getDiscard()) {
          ...
          if (parts.length > 1) {
            ...
            result.block.push(`${resultReg} = ${parts[0]};`);
          } else {
            result.block.push(`${resultReg} = ${language.assertionSuccess};`);
          }
        }
        result.block.push(language.blockEnd(label));
    ...
    discardable: buildDiscardablePredicate,
    discardable_and: buildDiscardablePredicate,
    discardable_not: buildDiscardablePredicate,
