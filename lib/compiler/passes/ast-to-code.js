  ...
  var generate = visitor.build({
    ...
    sequence: function(node, context) {
      if (node.elements.length === 1) {
        ...
        if (context.getDiscard()||context.getVoided()) {
    ...
    labeled: function(node, context) {
      ...
      var newContext;
      if (node.label === 'void') {
        newContext = context.silence().cloneEnv().voided();
        subresult = recurse(node.expression, newContext);
        return subresult;
      }
      var reg = allocReg([]);
      newContext = context.cloneEnv().resultReg(reg).discard(false);
      subresult = recurse(node.expression, newContext);
